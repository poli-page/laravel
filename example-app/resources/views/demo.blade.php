...
<title>Poli Page · Laravel demo</title>
...
<div class="page">
  ...
  <header class="mast">
    ...
    <p class="tagline">Your package, your dashboard. Every SDK feature, one click away.</p>
    <div class="status-row">
      <span class="dot"></span>
      <span>poli-page/laravel v0.1.0</span>
    </div>
  </header>
  ...
  <section>
    <div class="head">
      ...
      <p class="desc">
        <code>PoliPage\renderToFile()</code> streams the PDF straight to disk under
        <code>example-app/storage/poli-page/welcome.pdf</code> — memory-bounded regardless of size.
      </p>
    </div>
    <div class="actions">
  ...
  <section>
    <div class="head">
      ...
      <p class="desc">
        ...
        <code>INVALID_VERSION_FORMAT</code>. The package's typed exceptions propagate the
  ...
  <section>
    <div class="head">
      ...
      <p class="desc">
        These two commands prove the package wires through to Laravel's Artisan console.
        Copy and run in a terminal alongside this server.
      </p>
    </div>
    <div class="cli">
      <button class="copy" data-copy="php artisan app:demo:render-to-file">Copy</button>
      <span class="prompt">$</span>php artisan app:demo:render-to-file
    </div>
    <div class="cli">
      <button class="copy" data-copy="php artisan poli-page:render --project=getting-started --template=welcome --template-version=1.0.0 --output=/tmp/smoke.pdf">Copy</button>
      <span class="prompt">$</span>php artisan poli-page:render --project=getting-started \
        --template=welcome --template-version=1.0.0 --output=/tmp/smoke.pdf
    </div>
  </section>
...
(() => {
  ...
  const paneHead = (label, status, klass) =>
  ...
  const actions = {
    ...
    'bad-version': async target => {
      const r = await fetch('/api/errors/bad-version');
      const body = await r.text();
